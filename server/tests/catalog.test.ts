...
describe('signed catalog adapter', () => {
  it('verifies the Ed25519 envelope and puzzle digest', async () => {
    ...
    const puzzles = Buffer.from(JSON.stringify([{ id: 'puzzle-1' }, { id: 'puzzle-2' }]))
    ...
    assert.deepEqual(page, { items: [{ id: 'puzzle-1' }], nextCursor: '1' })
    assert.deepEqual((await service.listPuzzles({ packId: 'pack-e4', cursor: page.nextCursor!, limit: 20 })).items, [{ id: 'puzzle-2' }])
