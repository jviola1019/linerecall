...
- Better Auth configuration for five-minute, hashed, single-use magic links and session-gated passkeys. Every auth route is limited to 120/IP/five minutes; magic-link submission is further limited to 20/IP/hour and 5/normalized-email/hour, and passkey routes to 30/IP/five minutes. Exhausted magic-link limits return the same generic success shape. The production database and auth database use separate credentials.
