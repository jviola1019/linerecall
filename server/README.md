...
- Better Auth configuration for five-minute, hashed, single-use magic links and session-gated passkeys. Magic-link submission is limited to 20/IP/hour and 5/normalized-email/hour; exhausted email limits return the same generic success shape. The production database and auth database use separate credentials.
